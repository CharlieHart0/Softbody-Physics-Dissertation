...
void PhysicsObject::ConstrainVelocities() {
	if (constraints == ConstrainNone) {	return;	}

	if (constraints & ConstrainPositionX) { linearVelocity.x = 0;}
	if (constraints & ConstrainPositionY) { linearVelocity.y = 0;}
	if (constraints & ConstrainPositionZ) { linearVelocity.z = 0;}

	if (constraints & ConstrainRotationX) { angularVelocity.x = 0; }
	if (constraints & ConstrainRotationY) { angularVelocity.y = 0; }
	if (constraints & ConstrainRotationZ) { angularVelocity.z = 0; }
...
void PhysicsObject::SetConstraints(PhysicsObjectConstraints value) {
	constraints = value;
}
void PhysicsObject::SetConstraint(PhysicsObjectConstraints bit, bool value) {
	if (value) {
		constraints = (PhysicsObjectConstraints)((uint8_t)constraints | (uint8_t)bit);
	}
	else {
		constraints = (PhysicsObjectConstraints)((uint8_t)constraints - (uint8_t)bit);
	}
...
PhysicsObjectConstraints PhysicsObject::GetConstraints() {
	return constraints;
}
bool PhysicsObject::GetConstraint(PhysicsObjectConstraints bit) {
	return (uint8_t)constraints & (uint8_t)bit;
}

void PhysicsObject::SetIsTrigger(bool value) {
	isTrigger = value;
}
