		...
		protected:
			...
			void Insert(T& object, const Vector3& objectPos, const Vector3& objectSize, int depthLeft, int maxSize) {
				if(!CollisionDetection::AABBTest(objectPos, 
					...
					objectSize, 
					Vector3(size.x, 100.0f, size.y))) { return; }

				if (children) { // not leaf node, descend tree
					for (int i = 0; i < 4; i++) {
						children[i].Insert(object, objectPos, objectSize, depthLeft - 1, maxSize);
					}
				}

				else { // currently leaf node, can expand
					...
					if ((int)contents.size() > maxSize && depthLeft > 0) {
						if (!children) {
							...
							contents.clear(); // clear contents of current node because contents sent to new children nodes
						}
					}
				}
			}
