...
#define minimumBounceForce 2.5f
...
void PhysicsSystem::UpdateCollisionList() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = allCollisions.begin(); i != allCollisions.end(); ) {
		...
		if (!i->a->GetPhysicsObject()->GetIsTrigger()
			&& !i->b->GetPhysicsObject()->GetIsTrigger()) {
			if ((*i).framesLeft < 0) {
		...
		else { // support "collisions" with triggers, which will not push out collided objects
			CollisionDetection::CollisionInfo info;
			if (!CollisionDetection::ObjectIntersection(i->a, i->b, info)) {
				i->a->OnCollisionEnd(i->b);
				i->b->OnCollisionEnd(i->a);
				i = allCollisions.erase(i);
			}
	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info)) {
				if (!(*j)->GetPhysicsObject()->GetIsTrigger() &&
					!(*i)->GetPhysicsObject()->GetIsTrigger()) {
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if (totalMass == 0) { return; } // two static objs

	//separate using projection method
	...
	if (impulseForce < minimumBounceForce && impulseForce > -minimumBounceForce) { cRestitution = 0; }
...
void PhysicsSystem::IntegrateAccel(float dt) {
	...
	for (auto i = first; i != last; i++) {
		...
		if (object == nullptr) { continue; } //no physics object on this gameobject
		...
		//dont move objects with infinite mass
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	for (auto i = first; i != last; i++) {
		...
		object->ConstrainVelocities();
		if (object->GetConstraints() == ConstrainAll) { continue; }
...
void PhysicsSystem::ClearForces() {
	gameWorld.OperateOnContents(
		[](GameObject* o) {
			if (o->GetPhysicsObject() == NULL) { return; }
