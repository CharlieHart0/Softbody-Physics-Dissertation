...
void MeshGeometry::SetVertexPosition(unsigned int t_index, Vector3 t_pos)
{
	if (t_index > positions.size() - 1)
	{
		throw std::logic_error(__FUNCTION__ ": given index out of vector bounds!");
	}
	positions[t_index] = t_pos;
}
...
MeshGeometry::MeshGeometry(const std::string&filename) {
	...
	if (file.fail())
	{
		throw std::logic_error(__FUNCTION__ ": Mesh File \"" + filename + "\" does not exist!");
	}
...
void NCL::MeshGeometry::SetVertexNormal(unsigned int t_index, Vector3 t_norm)
{
	normals[indices[t_index]] = t_norm;
}
