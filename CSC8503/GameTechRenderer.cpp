...
void GameTechRenderer::LoadSkybox() {
	...
	string filenames[6] = {
		"/Cubemap/CloudyCrown_Midday_Front.png",
		"/Cubemap/CloudyCrown_Midday_Back.png",
		"/Cubemap/CloudyCrown_Midday_Up.png",
		"/Cubemap/CloudyCrown_Midday_Down.png",
		"/Cubemap/CloudyCrown_Midday_Right.png",
		"/Cubemap/CloudyCrown_Midday_Left.png"
	};
...
void GameTechRenderer::SortObjectList() {
	...
	for (const auto& object : activeObjects) {
		if (object->isAnimated()) { 
			animatedObjects.emplace_back(object);
		} else {
...
void GameTechRenderer::RenderCamera() {
	...
	for (const auto& obj : animatedObjects) {
		...
		if (activeShader != shader) {
			...
			joints = glGetUniformLocation(shader->GetProgramID(), "joints");
			evilAssetLocation = glGetUniformLocation(shader->GetProgramID(), "evilAsset");
		...
		Transform offsetTransform = *(*obj).GetTransform();
		Vector3 offsetPos = offsetTransform.GetPosition();
		offsetPos += obj->getOffsetPos();
		offsetTransform.SetPosition(offsetPos);

		Matrix4 modelMatrix = offsetTransform.GetMatrix();
		...
		glUniform1i(evilAssetLocation, obj->getIsEvilAsset());

		vector<Matrix4> invBindPose = obj->GetMesh()->GetInverseBindPose();
		const Matrix4* frameData = obj->GetCurrentAnimation()->GetJointData(obj->GetCurrentAnimation()->GetCurrentFrame());
		const int* bindPoseIndices = obj->GetMesh()->GetBindPoseIndices();

		if(activeShader != nullptr ) joints = glGetUniformLocation(shader->GetProgramID(), "joints");

		for (int i = 0; i < obj->GetMesh()->GetSubMeshCount(); ++i) {

			const MeshMaterialEntry* matEntry = obj->GetMat()->GetMaterialForLayer(i);
			TextureBase* texture = matEntry->GetEntry("Diffuse");
			BindTextureToShader(texture, "mainTex", 0);

			SubMeshPoses pose;
			obj->GetMesh()->GetBindPoseState(i, pose);
			...
			glUniformMatrix4fv(joints, frameMatrices.size(), false, (float*)frameMatrices.data());
			BindMesh((*obj).GetMesh());
			DrawBoundMesh(i);
		}
	}

	for (const auto&i : staticObjects) {
		...
		glUniform1i(evilAssetLocation, i->getIsEvilAsset());

		BindMesh((*i).GetMesh());
		int layerCount = (*i).GetMesh()->GetSubMeshCount();
		MeshMaterial* material = i->GetMat();
...
MeshMaterial* GameTechRenderer::LoadMaterial(const string& name) {
	MeshMaterial* material = new MeshMaterial(name);
	material->LoadTextures();
	return material;
}

MeshAnimation* GameTechRenderer::LoadAnimation(const string& name) {
	MeshAnimation* animation = new MeshAnimation(name);
	return animation;
}
...
void NCL::CSC8503::GameTechRenderer::RenderUI()
{
	ui->UpdateGameUI(0);
}
